        ...
        switch (req.method) {
            case 'GET':
                if (isNaN(id)) {
                    ...
                    if (!found) {
                        res.writeHead(404);
                        res.end(`Error : The bookmark with the id: ${id} is not found`);
                    }
                }
                break;
            case 'POST':
                let newBookmark = await getPayload(req);
                validateStatus = validateBookmark(newBookmark);
                if (validateStatus) {
                    let maxId = 0;
                ...
                if (!isNaN(id)) {
                    ...
                    for (let bookmark of bookmarks) {
                        if (bookmark.Id === id) {
                            ...
                            found = true;
                            bookmarks = bookmarks.filter(elem => elem != bookmark);
                            console.log(bookmarks);
                            fs.writeFileSync(bookmarksFilePath, JSON.stringify(bookmarks));
                            res.writeHead(204);
                            res.end();
            ...
            case 'PUT':
                ...
                validateStatus = validateBookmark(modifiedBookmark);
                if (validateStatus) {
                    if (!isNaN(id)) {
